...
| `FEED_URL_ALLOWLIST` | Comma-separated exact hostnames permitted to resolve to private/internal IPs (self-hosted feed sources on LAN/VPN, e.g. RSSify, FreshRSS, Miniflux) | | Skips the private hostname/private-IP rejection for allowlisted hosts only. Subdomains are **not** covered.
