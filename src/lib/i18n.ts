...
const dict = {
  ...
  'integration.autoSummary': { ja: '自動要約', en: 'Automatic summarization', zh: '自动摘要'},
  'integration.autoSummaryDesc': {
    ja: '新しく取得した記事に自動で要約を生成します。セマンティック検索（埋め込み）の前提条件です',
    en: 'Automatically generate summaries for newly fetched articles. This is a prerequisite for semantic search (embeddings)',
    zh: '自动为新获取的文章生成摘要。这是语义搜索（嵌入）的前提条件',
  },
  ...
  'settings.semanticSearch': { ja: 'セマンティック検索（埋め込み）', en: 'Semantic Search (embeddings)', zh: '语义搜索（嵌入）'},
  'settings.semanticSearchDesc': {
    ja: '記事のタイトルと要約をベクトル化して Meilisearch に埋め込み、キーワード + セマンティックのハイブリッド検索を行います。埋め込みは記事のタイトルと要約のみを入力とし、自動要約（上記の「要約」設定）が有効で設定されている場合にのみ有効化できます',
    en: 'Embed article titles and summaries in Meilisearch for hybrid keyword + semantic search. Embeddings use only each article\'s title and summary, and can only be enabled when automatic summarization above is configured and enabled',
    zh: '将文章标题和摘要嵌入 Meilisearch，进行关键词 + 语义混合搜索。嵌入仅使用文章的标题和摘要，且仅当上方自动摘要已配置并启用时才能开启',
  },
  'settings.semanticEnable': { ja: 'セマンティック検索', en: 'Semantic search', zh: '语义搜索'},
  'settings.semanticDependencyNote': {
    ja: '有効にするには、上の「要約」欄で自動要約を ON にし、APIキー付きのプロバイダーとモデルを選択してください。セマンティック検索は自動要約に依存します',
    en: 'To enable this, turn ON automatic summarization above and pick a provider with an API key plus a model. Semantic search depends on automatic summarization',
    zh: '如需启用，请在上方「摘要」中打开自动摘要，并选择已配置 API 密钥的提供商和模型。语义搜索依赖自动摘要',
  },
  'settings.semanticPrerequisiteFailed': {
    ja: 'セマンティック検索を有効にできません: ${reason}',
    en: 'Cannot enable semantic search: ${reason}',
    zh: '无法启用语义搜索：${reason}',
  },
  'settings.semanticProvider': { ja: '埋め込みプロバイダー', en: 'Embedding provider', zh: '嵌入提供商'},
  'settings.semanticProviderOpenai': { ja: 'OpenAI（クラウド）', en: 'OpenAI (cloud)', zh: 'OpenAI（云端）'},
  'settings.semanticProviderOllama': { ja: 'Ollama（ローカル）', en: 'Ollama (local)', zh: 'Ollama（本地）'},
  'settings.semanticModel': { ja: '埋め込みモデル', en: 'Embedding model', zh: '嵌入模型'},
  'settings.semanticModelHint': {
    ja: '英語の検索を想定したモデルを推奨（例: text-embedding-3-small, nomic-embed-text）。v1 では日本語・英語間の意味検索保証はありません',
    en: 'English-focused models recommended (e.g. text-embedding-3-small, nomic-embed-text). v1 does not promise cross-language retrieval',
    zh: '建议使用面向英文的模型（例如 text-embedding-3-small、nomic-embed-text）。v1 不保证跨语言检索',
  },
  'settings.semanticDimensions': { ja: '次元数（任意）', en: 'Dimensions (optional)', zh: '维度（可选）'},
  'settings.semanticDimensionsDesc': {
    ja: 'モデルの出力次元に合わせてください。未指定時はモデル既定値を使用',
  ...
  'settings.semanticEmbeddedProgress': {
    ...
    zh: '已嵌入 ${embedded} / ${documents} 篇文章',
  },
  'settings.semanticRebuildButton': { ja: 'セマンティックインデックスを再構築', en: 'Rebuild semantic index', zh: '重建语义索引'},
  'settings.semanticRebuildConfirm': {
    ja: '全記事（タイトル+要約）を対象に埋め込みを再生成します。プロバイダーの API 課金が発生する可能性があります。続行しますか？',
    en: 'This regenerates embeddings for every article (title + summary) and may incur provider API costs. Continue?',
    zh: '这将为所有文章（标题 + 摘要）重新生成嵌入，可能产生提供商 API 费用。是否继续？',
  },
  'settings.semanticRebuildStarted': { ja: '再構築を開始しました', en: 'Rebuild started', zh: '已开始重建'},
  'settings.semanticLastRebuildFailed': {
    ja: '直近の再構築に失敗しました: ${error}',
    en: 'Last rebuild failed: ${error}',
    zh: '上次重建失败：${error}',
  },
  'settings.semanticTestConnection': { ja: '接続テスト', en: 'Test connection', zh: '测试连接'},
  'settings.semanticTesting': { ja: 'テスト中…', en: 'Testing…', zh: '测试中…'},
  'settings.semanticTestOk': {
    ja: '接続OK（${model}, ${dimensions}次元）',
    en: 'Connection OK (${model}, ${dimensions} dims)',
    zh: '连接正常（${model}，${dimensions} 维）',
  },
  'settings.semanticSave': { ja: '保存', en: 'Save', zh: '保存'},
  'settings.semanticSaved': { ja: '保存しました。インデックスを再構築します…', en: 'Saved. Rebuilding index…', zh: '已保存。正在重建索引…'},
  ...
  'search.semanticFallback': {
    ja: 'セマンティック検索が一時的に利用できないため、キーワード検索の結果を表示しています',
    en: 'Semantic search temporarily unavailable — showing keyword results',
    zh: '语义搜索暂时不可用 — 显示关键词搜索结果',
  },
